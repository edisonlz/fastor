$(document).ready(function() {
  ...
  SquireUI = function(options) {
    ...
    $(div).load(options.buildPath + 'Squire-UI.html', function() {
      ...
      this.videoDrop = new Drop({
        target: $('#insertVideo').first()[0],
        content: $('#drop-video').html(),
        position: 'bottom center',
        openOn: 'click'
      });

      this.videoDrop.on('open', function () {
        $('.quit').unbind().click(function () {
          $(this).parent().parent().removeClass('drop-open');
        });

        $('.sumbitVideoURL').unbind().click(function () {
          console.log("Passed through .sumbitVideoURL");
          var editor = iframe.contentWindow.editor;
          url = $(this).parent().children('#videoUrl').first()[0];
          editor.insertVideo(url.value);
          console.info('...' + url.value);
          $(this).parent().parent().removeClass('drop-open');
          $(this).parent().children('#videoUrl').attr('value', '');
        });
      ...
      $('.item').click(function() {
        ...
        } else if (test.isNotValue('makeLink') | test.isNotValue('insertImage') | test.isNotValue('insertVideo') | test.isNotValue('selectFont')) {
