...
proto.insertVideo = function ( src, attributes ) {
    // TODO: request video info and add pic to web
    console.info('insertVideo: ' + src + "; attrs: " + attributes);
    var dom = this;
    var img = null;
    $.ajax({
        ...
        success: function (data) { //请求成功的回调函数
            var img_url = data.h_image;
            if (data.video_type == 'show') {
              img_url = data.v_image
            }

            img = dom.createElement( 'IMG', mergeObjects({
                src: img_url
            }, attributes ));
            dom.insertElement( img );
        }
//        complete:function(XMLHttpRequest, status){
//            if(status == 'timeout'){
//                verify_url_func.abort();
//                $().toastmessage('showToast', {text: "超时, 请重试😢～", type: 'error', sticky: false});
