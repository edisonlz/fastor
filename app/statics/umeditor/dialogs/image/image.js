(function () {
    ...
    var utils = UM.utils,
        browser = UM.browser,
        edit_width = $('.edui-body-container').width();
        Base = {
        checkURL: function (url) {
            ...
            if (!/(.gif|.jpg|.jpeg|.png)$/i.test(url) && !/ykimg.com/.test(url)) {
                return false;
            }
            return url;
        },
...
//                var $img = $("<img src='" + editor.options.imagePath + url + "' class='edui-image-pic' />"),
    ...
    var Upload = {
        ...
        uploadTpl: '<div class="edui-image-upload%%">' +
            ...
            '<input style=\"filter: alpha(opacity=0);\" class="edui-image-file" type="file" hidefocus name="files[]" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp"/>' +
            '</form>' +
        ...
        submit: function (callback) {
            ...
            var me = this,
                input = $( '<input style="filter: alpha(opacity=0);" class="edui-image-file" type="file" hidefocus="" name="files[]" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp">'),
                input = input[0];
            ...
            $(me.dialog).delegate( ".edui-image-file", "change", function ( e ) {
                ...
                $('<iframe name="up"  style="display: none"></iframe>').insertBefore(me.dialog).on('load', function(){
                    var r1 = this.contentWindow.document.body.innerHTML;
                    var r = this.contentWindow.document.body.innerText;
                    r = (r1 || r).replace(/<\/?pre.*?>/g,'');
                    if(r == '')return;
                    me.uploadComplete(r);
                    $(this).unbind('load');
                    $(this).remove();
            ...
            'ok': {
                exec: function (editor, $w) {
                    ...
                    if (index != -1) {
                        ...
                        $("<img/>").attr("src", list[0].src).load(function() {
