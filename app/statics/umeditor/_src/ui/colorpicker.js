...
UM.ui.define('colorpicker', {
    tpl: function (opt) {
        var COLORS = ('DE704F,FF5F2E,FF501B,8B572A,DE704F,FF5F2E,FF501B,8B572A,DE704F,FF501B,' +
        ...
        for (var i = 0; i < COLORS.length; i++) {
            if (i && i % 10 === 0) {
                html += '</tr>' + (i == 70 ? '<tr><td colspan="10">'+opt.lang_standardColor+'</td></tr>' : '') + '<tr' + (i == 60 ? ' class="edui-colorpicker-firstrow"' : '') + '>';
            }
            html += i < 80 ? '<td><a unselectable="on" onmousedown="return false" title="' + COLORS[i] + '" class="edui-colorpicker-colorcell"' +
                ' data-color="#' + COLORS[i] + '"' +
                ' style="background-color:#' + COLORS[i] + ';border:solid #ccc;' +
                (i < 20 || i >= 70 ? 'border-width:1px;' :
                    i >= 20 && i < 30 ? 'border-width:1px 1px 0 1px;' :
