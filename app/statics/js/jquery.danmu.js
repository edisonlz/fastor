...
(function( $ ){
	...
	cyntax.plugins.timer.prototype = {
		...
		resume : function() {
			this.timer_info.state = 1;
			this.timer_id = setTimeout( $.proxy( this._timer_fn, this ) , 10);
		}
	};
...
;(function( $ ){
	...
	var Danmu= function (element, options) {
		...
		this.$timer.timer({
			...
			callback: function( index ) {
                ...
                if($(element).data("danmuPlayList")[$(element).data("nowTime")]==1){
						return;
				}

				var temTime = 0 ;
				var temDA=$(element).data("danmuList")[(parseInt($(element).data("nowTime"))).toString()];
				...
				if($(element).data("danmuList")[$(element).data("nowTime")]){
					...
					var now = $(element).data("nowTime");

					$(element).data("danmuPlayList")[$(element).data("nowTime")] = 1;

					for(var i=0;i<danmus.length;i++){
						var _id = me.id+"tempDanmaku_" + now + "_" +i;

						var a_danmu="<div class='danmaku' id='"+_id+"'></div>";
						var key = "#"+_id;

						$(element).append(a_danmu);
						$(key).text(danmus[i].text);
						...
						if  ( danmus[i].position == 0){
							...
							var row = parseInt(danmakuOP*Math.random()+1) % danmakuOP;
							
							var top_local= row *options.FontSizeBig;
                            var left_local=$(key).width()+$(this).width();
	...
	Danmu.prototype.danmuPause = function(){
		$("#"+this.id+' .danmaku').velocity("finish");
	...
	Danmu.prototype.danmuResume = function(){
		...
		//$("#"+this.id+' .danmaku').velocity("reverse");
		this.$element.data("paused",0);
	};
